...
inline photo_plane_fit fit_photo_plane_to_reflections(
    ...
    for (int j = 0; j < image_h; ++j) {
        for (int i = 0; i < image_w; ++i) {
            ...
            min_x = std::fmin(min_x, hit_point.x());
            max_x = std::fmax(max_x, hit_point.x());
            min_z = std::fmin(min_z, hit_point.z());
            max_z = std::fmax(max_z, hit_point.z());
            ++samples;
        }
    }
...
inline void generate_inverse_mapped_photo_texture(
    ...
    // Resolve height ratio: <0 means "auto from target image's aspect" so the
    // image isn't squashed when placed on the cylinder.
    ...
    fill_empty_pixels(pixels, counts, texture_width, texture_height, 1);
